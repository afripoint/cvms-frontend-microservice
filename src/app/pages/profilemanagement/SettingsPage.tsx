...
const SettingsPage = () => {
  ...
  const handleTabChange = async (tabId: string) => {
    ...
    // If switching to Teams tab, fetch sub-accounts
    if (tabId === "Teams" && isBusinessAccount) {
      try {
        console.log("Teams tab clicked, fetching sub-accounts...")
        await dispatch(fetchSubAccounts())
      } catch (error) {
        console.error("Failed to fetch sub-accounts when switching to Teams tab:", error)
      }
    }
  }
